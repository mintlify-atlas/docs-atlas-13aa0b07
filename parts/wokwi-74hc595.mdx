...
description: 8-bit Serial-In Parallel-Out (SIPO) Shift Register for expanding output pins
---

8-bit Serial-In Parallel-Out (SIPO) Shift Register.

Use the 74HC595 shift register to expand the number of _output_ pins on your microcontroller. For input shift register (e.g. reading multiple buttons with a single input pin), please see the [wokwi-74hc165](/parts/wokwi-74hc165).
...
<ParamField path="SHCP" type="clock">
  Serial clock
</ParamField>

<ParamField path="STCP" type="control">
  Storage (latch) pin
</ParamField>
...
The output pins of the shift register, Q0 through Q7, are usually connected to [LEDs](/parts/wokwi-led) or a 7-segment display.

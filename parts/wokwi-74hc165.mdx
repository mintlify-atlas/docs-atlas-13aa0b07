...
description: 8-bit Parallel-In Serial-Out (PISO) Shift Register for expanding input pins
---

8-bit Parallel-In Serial-Out (PISO) Shift Register (Input).

Use the 74HC165 shift register to expand the number of _input_ pins on your microcontroller. For output shift register (e.g. controlling multiple LEDs with just a few pins), please see the [wokwi-74hc595](/parts/wokwi-74hc595).

## Pins

<ParamField path="D0…D7" type="input">
  Parallel input
</ParamField>

<ParamField path="PL" type="control">
  Parallel load (active low)
</ParamField>

<ParamField path="CP" type="clock">
  Serial clock
</ParamField>

<ParamField path="CE" type="control">
  Clock enable (active low)
</ParamField>

<ParamField path="Q7" type="output">
  Serial output
</ParamField>

<ParamField path="Q7_N" type="output">
  Inverted serial output (usually not used)
</ParamField>

<ParamField path="DS" type="input">
  Serial input - Use to daisy-chain multiple 74HC165 units together. Connect DS to the Q7 pin of the previous 74HC165 chip in the chain. You can leave DS disconnected if you don't chain or for the first chip in the chain.
</ParamField>
...
To use the shift register, connect pins D0…D7 to your inputs (e.g. [slide switches](/parts/wokwi-slide-switch) or [pushbuttons](/parts/wokwi-pushbutton)). You may need to add external pull-up or pull-down [resistors](/parts/wokwi-resistor), especially if you go with the buttons.
